...
def get_fn_name(label):
    ...
    if 'xxhash' in label:
        return 'XXHash'
    if 'mult_prime' in label:
        return 'MultiplyPrime'
    if 'fibonacci_prime' in label:
        return 'FibonacciPrime'

# ------- collision plot ------- #
def collisions(df):
    df = df[df["name"].str.lower().str.contains("collision")].copy(deep=True)
...
with open(file_path, 'r') as json_file:
    ...
    # compute dataset names
    df["dataset_name"] = df["dataset_id"].apply(lambda x : DATASET_NAMES[x//1])
