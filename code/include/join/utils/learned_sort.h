...
class RMI {
 public:
  ...
  struct Params {
    ...
    static constexpr unsigned int DEFAULT_BATCH_SZ = LS_DEFAULT_BATCH_SZ; //10
    static constexpr unsigned int DEFAULT_FANOUT = LS_DEFAULT_FANOUT;  //1e3
    static constexpr float DEFAULT_OVERALLOCATION_RATIO = LS_DEFAULT_OVERALLOCATION_RATIO; //1.1
    static constexpr float DEFAULT_SAMPLING_RATE = LS_DEFAULT_SAMPLING_RATE; //.01
    static constexpr unsigned int DEFAULT_THRESHOLD = LS_DEFAULT_THRESHOLD; //100
    vector<unsigned int> DEFAULT_ARCH = {1, LS_DEFAULT_ARCH_SECOND_LEVEL}; //1000
    static constexpr unsigned int MIN_SORTING_SIZE = LS_MIN_SORTING_SIZE; //1e4
  ...
  vector<T> tmp_training_sample;
  vector<T> sorted_training_sample;
  Params hp;
...
RMI<typename iterator_traits<RandomIt>::value_type> train(
    RandomIt, RandomIt,
    typename RMI<typename iterator_traits<RandomIt>::value_type>::Params &, int, int);

// Default comparison function [std::less()] and default hyperparameters
// Drop-in replacement for std::sort()
template <class RandomIt>
void sort(RandomIt, RandomIt, int, int);
...
// Default comparison function and custom RMI
template <class RandomIt>
void sort(
    RandomIt, RandomIt,
    RMI<typename iterator_traits<RandomIt>::value_type> &, int, int);
 ...
 */
template <class RandomIt>
learned_sort::RMI<typename iterator_traits<RandomIt>::value_type> learned_sort::train(
    RandomIt begin, RandomIt end,
    typename RMI<typename iterator_traits<RandomIt>::value_type>::Params &p, int thread_id, int partition_id) {
  ...
  rmi.tmp_training_sample.reserve(SAMPLE_SZ);
  ...
  for (auto i = begin; i < end; i += offset) {
    ...
    rmi.tmp_training_sample.push_back(*i);
    //rmi.training_sample.push_back(*i);
  }

#ifdef USE_AVXSORT_AS_STD_SORT

 uint32_t tmp_training_sample_size = rmi.tmp_training_sample.size();
  T sorted_training_sample_arr[tmp_training_sample_size];
  int64_t * inputptr = (int64_t *)(&(rmi.tmp_training_sample[0]));
  int64_t * outputptr = (int64_t *)sorted_training_sample_arr;
  avxsort_int64(&inputptr, &outputptr, tmp_training_sample_size);
  int64_t * tmp_outputptr = (int64_t *) outputptr;
  rmi.sorted_training_sample.insert(rmi.sorted_training_sample.begin(), tmp_outputptr, tmp_outputptr + tmp_training_sample_size);
...
/*  uint32_t tmp_training_sample_size = rmi.tmp_training_sample.size(); 
  ...
  int64_t * outputptr = (int64_t *)sorted_training_sample_arr; 
  avxsort_int64(&inputptr, &outputptr, tmp_training_sample_size);
  rmi.sorted_training_sample.insert(rmi.sorted_training_sample.begin(), sorted_training_sample_arr, sorted_training_sample_arr + tmp_training_sample_size);
  rmi.training_sample = &(rmi.sorted_training_sample);*/
#else
...
void _sort_trained(RandomIt begin, RandomIt end,
                   learned_sort::RMI<typename iterator_traits<RandomIt>::value_type> &rmi, int thread_id, int partition_id) {
  // Determine the data type
  typedef typename iterator_traits<RandomIt>::value_type T;
...
/*  if(thread_id == 0 && partition_id == 0)
  {
    printf("major bckt sizes \n");
    for(int i = 0; i < major_bckt_sizes.size(); i++)
    {
      printf("major bckt %d: %d \n", i, major_bckt_sizes[i]);
    }
  }
*/
  ...
  for (unsigned int major_bckt_idx = 0; major_bckt_idx < FANOUT;
    ...
    for (unsigned int batch_idx = 0; batch_idx < num_batches; ++batch_idx) {
      ...
      for (unsigned int elm_idx = 0; elm_idx < BATCH_SZ; ++elm_idx) {
        ...
        {
          printf("cur_elm %ld root_slope %lf root_intrcpt %lf batch_cache[elm_idx] %d slopes[batch_cache[elm_idx]] %lf intercepts[batch_cache[elm_idx]] %lf pred_cdf %lf pred_cdf * TOT_NUM_MINOR_BCKTS %lf  major_bckt_idx * NUM_MINOR_BCKT_PER_MAJOR_BCKT %ld pred_cdf * TOT_NUM_MINOR_BCKTS - major_bckt_idx * NUM_MINOR_BCKT_PER_MAJOR_BCKT %lf NUM_MINOR_BCKT_PER_MAJOR_BCKT %ld batch_cache[elm_idx] %ld \n", 
                  cur_elm, root_slope, root_intrcpt, static_cast<int>(std::max(0.,std::min(num_models - 1., root_slope * cur_elm + root_intrcpt))), 
                  slopes[static_cast<int>(std::max(0., std::min(num_models - 1., root_slope * cur_elm + root_intrcpt)))], 
                  intercepts[static_cast<int>(std::max(0., std::min(num_models - 1., root_slope * cur_elm + root_intrcpt)))], 
                  pred_cdf, pred_cdf * TOT_NUM_MINOR_BCKTS, major_bckt_idx * NUM_MINOR_BCKT_PER_MAJOR_BCKT, pred_cdf * TOT_NUM_MINOR_BCKTS -
                             major_bckt_idx * NUM_MINOR_BCKT_PER_MAJOR_BCKT, NUM_MINOR_BCKT_PER_MAJOR_BCKT, batch_cache[elm_idx]);
        }*/
      }
    ...
    /*if(thread_id == 0 && partition_id == 0 && major_bckt_idx <= 25)
    {
      ...
      for(int i = 0; i < minor_bckt_sizes.size(); i++)
      {
...
if(thread_id==0 && partition_id==0)
  ...
  T sorted_spill_bucket_arr [spill_bucket_size];
  ...
  vector<T> sorted_spill_bucket(outputptr, outputptr + spill_bucket_size);
  ...
  for (unsigned int val_idx = 0; val_idx < EXCEPTION_VEC_INIT_CAPACITY; ++val_idx) {
  ...
  #ifdef USE_AVXSORT_AS_STD_SORT 
    #ifdef USE_AVXMERGE_AS_STD_MERGE
      avx_merge_int64((int64_t*) (&major_bckts[0]), (int64_t *)(&((*spill_bucket)[0])), ((int64_t *)begin) + total_repeated_keys, num_tot_elms_in_bckts, spill_bucket_size);
    #else
...
void learned_sort::sort(
    ...
    typename RMI<typename iterator_traits<RandomIt>::value_type>::Params
        &params, int thread_id, int partition_id) {
  ...
  if (std::distance(begin, end) <=
    ...
    // Train
    RMI rmi = train(begin, end, params, thread_id, partition_id);
...
void learned_sort::sort(
    RandomIt begin, RandomIt end,
    RMI<typename iterator_traits<RandomIt>::value_type> &rmi, int thread_id, int partition_id) {
  // Use std::sort for very small arrays
  if (std::distance(begin, end) <=
      std::max(rmi.params.fanout * rmi.params.threshold, 5 * rmi.params.arch[1])) {
    std::sort(begin, end);
  } else {
    if (rmi.trained)
      _sort_trained(begin, end, rmi, thread_id, partition_id); 
 ...
 */
template <class RandomIt>
void learned_sort::sort(RandomIt begin, RandomIt end, int thread_id, int partition_id) {
  typename RMI<typename iterator_traits<RandomIt>::value_type>::Params p;
  learned_sort::sort(begin, end, p, thread_id, partition_id);
}
